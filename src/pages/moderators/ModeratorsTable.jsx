...
export default function ModeratorsTable({
  ...
  console.log("ModeratorsTable -> ModeratorsData", ModeratorsData);
  const dropdownRefs = useRef({});
  const { t } = useI18nContext();

  const [selectedModeratorId, setSelectedModeratorId] = useState(null);

  const toggleEditDropdown = (moderatorId) => {
    setSelectedModeratorId((prevModeratorId) =>
      prevModeratorId === moderatorId ? null : moderatorId
    );
  };

  const handleClickOutside = (event, moderatorId) => {
    const dropdown = dropdownRefs.current[moderatorId];
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      handleClickOutside(event, selectedModeratorId);
    };
  ...
  const handlePreviewClick = (moderator) => {
    openPreview(moderator);
  };
                    ...
                    {ModeratorsData &&
                      ModeratorsData.map((moderator) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">{moderator.name}</td>
                          ...
                          <td className="px-4 py-3">{moderator.gender}</td>
                          <td className="px-4 py-3 flex items-center justify-end">
                            <button
                              ...
                              type="button"
                              onClick={() => toggleEditDropdown(moderator.id)}
                              ref={(el) =>
                                (dropdownRefs.current[moderator.id] = el)
                              }
                            >
                            ...
                            <div className="absolute z-50">
                              <div
                                id={`moderator-dropdown-${moderator.id}`}
                                className={`${
                                  selectedModeratorId === moderator.id
                              ...
                              >
                                <ul className="py-1 text-sm">
                                  ...
                                  <li>
                                    <button
                                      type="button"
                                      onClick={() => handlePreviewClick(moderator)}
                                      className="flex gap-2 w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200"
                                    >
