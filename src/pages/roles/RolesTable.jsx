...
export default function RolesTable({ RolesData, openCreate, openPreview }) {
  console.log("RolesTable -> RolesData", RolesData);
  const dropdownRefs = useRef({});
  const { t } = useI18nContext();

  const [selectedRoleId, setSelectedRoleId] = useState(null);

  const toggleEditDropdown = (roleId) => {
    setSelectedRoleId((prevRoleId) => (prevRoleId === roleId ? null : roleId));
  };

  const handleClickOutside = (event, roleId) => {
    const dropdown = dropdownRefs.current[roleId];
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      handleClickOutside(event, selectedRoleId);
    };
                        ...
                        >
                          ...
                          <td className="px-4 py-3 flex items-center justify-end">
                            <button
                              ...
                              type="button"
                              onClick={() => toggleEditDropdown(role.id)}
                              ref={(el) => (dropdownRefs.current[role.id] = el)}
                            >
                              <DotsThree size={25} weight="bold" />
                            </button>
                            <div className="absolute z-50">
                              <div
                                id={`role-dropdown-${role.id}`}
                                className={`${
