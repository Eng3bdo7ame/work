...
export default function StudentsTable({ openCreate, openPreview }) {
  ...
  const toggleEditDropdown = (studentId) => {
    setSelectedStudentId((prevStudentId) =>
      prevStudentId === studentId ? null : studentId
    );
  };

  const handleClickOutside = (event, studentId) => {
    const dropdown = dropdownRefs.current[studentId];
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      handleClickOutside(event, selectedStudentId);
    };
                      ...
                      >
                        ...
                        <td className="px-4 py-3 flex items-center justify-end">
                          <button
                            ...
                            type="button"
                            onClick={() => toggleEditDropdown(student.id)}
                            ref={(el) =>
                          ...
                          <div className="absolute z-50">
                            <div
                              id={`student-dropdown-${student.id}`}
                              className={`${
