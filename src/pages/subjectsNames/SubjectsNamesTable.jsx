...
export default function SubjectsNamesTable({
  SubjectsNamesData,
  openCreate,
  openPreview,
}) {
  ...
  const [selectedSubjectNameId, setSelectedSubjectNameId] = useState(null);

  const toggleEditDropdown = (subjectNameId) => {
    setSelectedSubjectNameId((prev) => (prev === subjectNameId ? null : subjectNameId));
  };

  const handleClick = (event, subjectId) => {
    const dropdown = dropdownRefs.current[subjectId];
    if (dropdown && !dropdown.contains(event.target) && !event.target.classList.contains("edit-button")) {
      setSelectedSubjectNameId(null);
    }
  };

  useEffect(() => {
    const handleOutsideClick = (event) => {
      handleClick(event, selectedSubjectNameId);
    };
            ...
            <div className="bg-white dark:bg-gray-800 relative shadow-md rounded-lg">
              <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                ...
                <button
                  ...
                  >
                    ...
                    {t("subjectsForm.subjectsNameForm.createSubjectName")}
                  </button>
                </div>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                  ...
                  <tbody>
                    {SubjectsNamesData.map((subjectName) => (
                      <tr
                      ...
                      >
                        ...
                        <td className="px-4 py-3">{subjectName.name}</td>
                        ...
                        <td className="px-4 py-3 flex items-center justify-end">
                          <button
                            ...
                            type="button"
                            onClick={() => toggleEditDropdown(subjectName.id)}
                            ref={(el) =>
                              (dropdownRefs.current[subjectName.id] = el)
                            }
                          >
                          ...
                          <div className="absolute z-50">
                            <div
                              id={`subject-dropdown-${subjectName.id}`}
                              className={`${
                                selectedSubjectNameId === subjectName.id
