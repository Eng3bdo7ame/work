...
export default function CreateSubjectName({
  refreshData,
  closeModal,
  subjectName,
  modal,
  ...
  const createSubjectName = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      const newSubjectName = response.data;
  ...
  const handleSubjectNameSelect = (selectedSubjectName) => {
    setSelectedSubjectName((prevSelectedSubjectsNames) => [
      ...prevSelectedSubjectsNames,
      {
        label: selectedSubjectName.label,
      },
    ]);
  };

  const handleSubjectNameRemove = (removedSubjectName) => {
    setSelectedSubjectName((prevSelected) =>
      prevSelected.filter((subjectName) => subjectName !== removedSubjectName)
    );
  };
      ...
      >
        ...
        >
          ...
          <div className="relative p-4 bg-white dark:bg-gray-800 sm:p-5">
            ...
            {/* Modal body */}
            <form onSubmit={createSubjectName}>
              <div className="grid gap-4 mb-4 sm:grid-cols-1">
                {/* اسم المادة */}
                <FormText
                  label={t("subjectsForm.subjectsNameForm.subjectName")}
                  name="subjectName"
                  value={formData.subjectName}
                  placeholder={t("subjectsForm.subjectsNameForm.subjectName")}
                  required=""
