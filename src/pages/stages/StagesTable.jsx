...
export default function StagesTable({ StagesData, openCreate, openPreview }) {
  const dropdownRefs = useRef({});
  const { t } = useI18nContext();

  const [selectedStageId, setSelectedStageId] = useState(null);

  const toggleEditDropdown = (stageId) => {
    setSelectedStageId((prev) => (prev === stageId ? null : stageId));
  };

  const handleClick = (event, stageId) => {
    const dropdown = dropdownRefs.current[stageId];
    if (
      dropdown &&
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      handleClick(event, selectedStageId);
    };
                      ...
                      >
                        ...
                        <td className="px-4 py-3 flex items-center justify-end">
                          <button
                            ...
                            type="button"
                            onClick={() => toggleEditDropdown(stage.id)}
                            ref={(el) => (dropdownRefs.current[stage.id] = el)}
                          >
                            <DotsThree size={25} weight="bold" />
                          </button>
                          <div className="absolute z-50">
                            <div
                              id={`stage-dropdown-${stage.id}`}
                              className={`${
