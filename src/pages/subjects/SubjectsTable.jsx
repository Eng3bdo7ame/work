...
export default function SubjectsTable({
  ...
  console.log("SubjectsTable -> SubjectsData", SubjectsData);
  const dropdownRefs = useRef({});
  const { t } = useI18nContext();

  const [selectedSubjectId, setSelectedSubjectId] = useState(null);

  const toggleEditDropdown = (subjectId) => {
    setSelectedSubjectId((prevSubjectId) =>
      prevSubjectId === subjectId ? null : subjectId
    );
  };

  const handleClickOutside = (event, subjectId) => {
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      handleClickOutside(event, selectedSubjectId);
    };
                    ...
                    {SubjectsData &&
                      SubjectsData.map((subject) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">{subject.subImg}</td>
                          <td className="px-4 py-3">{subject.name}</td>
                          <td className="px-4 py-3">{subject.stage}</td>
                          <td className="px-4 py-3">{subject.class}</td>
                          ...
                          <td className="px-4 py-3 flex items-center justify-end">
                            <button
                              ...
                              type="button"
                              onClick={() => toggleEditDropdown(subject.id)}
                              ref={(el) =>
                                (dropdownRefs.current[subject.id] = el)
                              }
                            >
                            ...
                            <div className="absolute z-50">
                              <div
                                id={`subject-dropdown-${subject.id}`}
                                className={`${
