...
export default function CreateClass({ refreshData, closeModal, Class, modal }) {
  ...
  const createClass = async (e) => {
    ...
    try {
      const response = await api.post("/Classes/Add", formData, {
        headers: {
          ...
          <div className="relative p-4 bg-white dark:bg-gray-800 sm:p-5">
            ...
            <form onSubmit={createClass}>
              <div className="grid gap-4 mb-4 sm:grid-cols-1">
                {/* اسم المرحلة */}
                <FormText
                  label={t("classesForm.className")}
                  name="stage"
                  value={formData.class}
                  placeholder={t("classesForm.className")}
                  required=""
                ...
                {/* اسم التخصص */}
                <FormText
                  label={t("classesForm.divisionName")}
                  name="specialty"
                  value={formData.division}
                  placeholder={t("classesForm.divisionName")}
                  required=""
                  onChange={handleChange}
                />
              </div>
