...
export default function ClassesTable({ ClassesData, openCreate, openPreview }) {
  const dropdownRefs = useRef({});
  const { t } = useI18nContext();

  const [selectedClassId, setSelectedClassId] = useState(null);

  const toggleEditDropdown = (classId) => {
    setSelectedClassId((prev) => (prev === classId ? null : classId));
  };

  const handleClick = (event, classId) => {
    const dropdown = dropdownRefs.current[classId];
    if (
      dropdown &&
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      handleClick(event, selectedClassId);
    };
                      ...
                      >
                        ...
                        <td className="px-4 py-3 flex items-center justify-end">
                          <button
                            ...
                            type="button"
                            onClick={() => toggleEditDropdown(classItem.id)}
                          ...
                          <div className="absolute z-50">
                            <div
                              id={`subject-dropdown-${classItem.id}`}
                              className={`${
